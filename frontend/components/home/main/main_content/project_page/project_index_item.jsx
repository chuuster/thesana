...
export default class ProjectIndexItem extends React.Component {
  ...
  shouldComponentUpdate(nextProps) {
    if (
      document
        .querySelector(`[task="${this.props.task.id}"]`)
  ...
  toggleDone(id, value) {
    return (e) => {
      ...
      if (taskRow.classList.contains("incomplete-task")) {
        ...
        document
          .querySelector(`[checkid="${id}"]`)
          .classList.add("faded-check");
        setTimeout(() => {
          taskRow.classList.add("success");
        }, 2000);
      } else {
  ...
  render() {
    ...
    {
      this.state.done
        ? (checkClass = "mark-complete-check-button faded-check")
        : (checkClass = "mark-complete-check-button");
    }

    if (
      Number.isInteger(this.state.assignee_id) &&
      Object.keys(this.props.users).length > 1
    ) {
      userIcon = (
        <button className="user-circle-button">
          {this.props.users[this.state.assignee_id].initials}
        </button>
      );
    } else {
      userIcon = (
    ...
    !!this.state.due_date
      ? (dueDate = moment(this.state.due_date).format("MMM D"))
      : (dueDate = "");
    !!this.state.done
      ? (rowClass = "task-index-row completed-task task-hide")
      : (rowClass = "task-index-row incomplete-task");

    return (
      <div
        task={this.state.id}
        onClick={this.handleFormAreaClick(this.state.id)}
        className={rowClass}
      >
        <div className="task-index-row-left">
          <form>
            <div
              onClick={this.toggleDone(this.state.id, !this.state.done)}
              checkid={this.state.id}
            ...
            <input
              ...
              onKeyUp={(e) => {
                // create a new task and focus on that task
                if (e.keyCode === 13) {
                  this.props.addTask();
                }
              }}
        ...
        <div className="task-index-row-right">
          <div className="task-row-due-date">{dueDate}</div>
          <div className="task-row-assignee-icon">{userIcon}</div>
        </div>
      </div>
    );
  }
